...
public class SpeechRecognitionHelper {
    ...
    public static void run(Activity callingActivity) {
        ...
        if (isSpeechRecognitionActivityPresented(callingActivity) == true) {
            ...
            Toast.makeText(callingActivity, "In order to activate speech recognition you must install \"Google Voice Search\"",
     ...
     */
    public static void startRecognitionActivity(Activity callerActivity) {

        // creating an Intent with RecognizerIntent.ACTION_RECOGNIZE_SPEECH action
        Intent intent = new Intent(RecognizerIntent.ACTION_RECOGNIZE_SPEECH);

        // giving additional parameters:
        intent.putExtra(RecognizerIntent.EXTRA_PROMPT, "Speak now!");    // user hint
        ...
        callerActivity.startActivityForResult(intent, Constants.SPEECH_RECOGNITION_REQUEST_CODE);
    }

    /**
     * Asking the permission for installing Google Voice Search.
     * If permission granted sent user to Google Play
     * callerActivity Activity, that initialized installing
     */
    ...
    private static void installGoogleVoiceSearch(final Activity activity) {
        ...
        String title = "Install Voice Search from Google Play?";
        String msg = "For recognition it is necessary to install \"Google Voice Search\"" ;
